...
**Note:** Production URL should be set via `API_URL` environment variable.
...
- `bookingType`: `"hourly"`, `"daily"`, `"weekly"`, or `"monthly"` (optional, auto-selected based on duration)
...
// Daily Booking
totalAmount = space.pricePerDay × durationDays

// Weekly Booking
totalAmount = space.pricePerWeek × durationWeeks

// Monthly Booking
totalAmount = space.pricePerMonth × durationMonths
```
