...
export const getGoogleAuthUrlController = asyncHandler(async (req, res) => {
  try {
    ...
    if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_REDIRECT_URI) {
      throw new AppError('Google OAuth is not configured. Please set GOOGLE_CLIENT_ID and GOOGLE_REDIRECT_URI in your environment variables.', 500);
    }
...
export const googleCallback = asyncHandler(async (req, res) => {
  try {
    ...
    if (oauthError) {
      ...
      return res.redirect(
        `${frontendUrl}/auth/google/error?message=${encodeURIComponent('Google authentication failed')}`
      );
    }
