...
class AuthService {
  ...
  async register(userData, requestInfo) {
    try {
      const { email, password, fullname, phonenumber, role = 'user' } = userData;
  ...
  async login(loginData, requestInfo) {
    try {
      ...
      // Verify password
      const isPasswordValid = await authRepository.verifyPassword(user, password);
      if (!isPasswordValid) {
        throw new AppError('Incorrect email or password', 401);
      }
      ...
      const updatedUser = await authRepository.updateLoginStats(user._id, userAgent, ipAddress);
  ...
  async updateProfile(userId, updateData) {
    try {
      ...
      const updatedUser = await authRepository.updateUserById(userId, validatedData);
      if (!updatedUser) {
        throw new AppError('User not found', 404);
      }

      // Check if onboarding is complete
      if (!updatedUser.onboardingCompleted && this.isOnboardingComplete(updatedUser)) {
        await authRepository.updateUserById(userId, { onboardingCompleted: true });
  ...
  async changePassword(userId, passwordData) {
    try {
      ...
      const isCurrentPasswordValid = await authRepository.verifyPassword(userWithPassword, currentPassword);
  ...
  async requestEmailVerification(userId) {
    try {
      ...
      try {
        await emailService.sendEmailVerificationEmail(user.email, user.fullname, otp);
      } catch (error) {
  ...
  async googleAuth(googleData, requestInfo) {
    try {
      ...
      return formatOAuthResponse(user, {
        accessToken,
        refreshToken: refreshToken.token,
        expiresIn: 15 * 60
      }, 'google');
  ...
  validateProfileUpdates(updateData) {
    const allowedFields = ['fullname', 'phonenumber', 'profilePic', 'bio', 'location', 'preferences'];
    const validatedData = {};
